...
exports.getAllTutors = async (req, res) => {
  try {
    ...
    const rows = data.map((t) => ({
      ...
      isAgeConfirmed: Boolean(t.profile?.isAgeConfirmed),
      qualification: t.profile?.qualification || '',
      specialization: t.profile?.specialization || '',
      experience: t.profile?.experience ?? '',
      teachingMode: t.profile?.teachingMode || '',
      tuitionType: t.profile?.tuitionType || '',
      addressLine1: t.profile?.addressLine1 || '',
      city: t.profile?.city || '',
      state: t.profile?.state || '',
      pincode: t.profile?.pincode || '',
      subjects: t.profile?.subjects || [],
      classLevels: t.profile?.classLevels || [],
      boards: t.profile?.boards || [],
      exams: t.profile?.exams || [],
      studentTypes: t.profile?.studentTypes || [],
      groupSize: t.profile?.groupSize || '',
      groupSizes: t.profile?.groupSizes || [],
      hourlyRate: t.profile?.hourlyRate ?? '',
      monthlyRate: t.profile?.monthlyRate ?? '',
      ...
      isVerified: Boolean(t.profile?.isVerified),
      isFeatured: Boolean(t.profile?.isFeatured),
      rating: t.profile?.rating || 0,
      ratingCount: t.profile?.ratingCount || 0,
